...
from product_services import get_all_products
...
def render_main_screen(tk):
    Button(tk, text='Login', bg='light green', command=lambda: render_login_screen(tk)).grid(row=0, column=0)
...
def render_register_screen(tk):
    ...
    def on_register():
        ...
        if result:
            ...
            Label(tk, text='Invalid input!', fg='red').grid(row=3, column=1)
...
def render_login_screen(tk):
    ...
    def on_login():
        ...
        if result:
            render_shop_screen(tk)
        else:
            Label(tk, text='Invalid input!', fg='red').grid(row=2, column=1)

    Button(tk, text='Login', command=lambda: on_login()).grid(row=3, column=1)


def render_shop_screen(tk):
    clear_screen(tk)
